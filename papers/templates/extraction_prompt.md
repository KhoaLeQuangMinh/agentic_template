...
- **Mathematical Equations:** (Provide exact LaTeX formulations for key equations, attention mechanisms, normalization, and loss functions)
- **Loss Function(s):** (Exact mathematical formulation and weightings)
- **Tensor Dimensions & Data Flow:** (Input dimensions, intermediate feature maps, output dimensions, data types)

---

## 3. Training Setup & Hyperparameters
- **Optimizer & Schedule:** (e.g., AdamW, learning rate, warmup steps, cosine decay schedule)
- **Hyperparameter Specifics:** (Batch size, epoch/step count, weight decay, dropout rate)
- **Datasets Used & Preprocessing Rules:** (Normalization parameters, data split ratios, augmentations)

---

## 4. Benchmark Results & Key Claims
- **Baseline Comparison:** (Models compared against)
- **Key Metrics Achieved:** (Accuracy, F1, AUC, Loss, Inference Speed)
- **Ablation Study Takeaways:** (Which components contributed most to performance)

---

## 5. Implementation & Codebase Mapping
- **Cloned Repo Reference:** Path inside `/repos/<repo_name>/` (if cloned).
- **Core Files in Reference Repo:** List specific files/classes in `/repos/` that implement the core mathematical claims.
- **Experiment Adaptation Strategy:** How can we import or modularly subclass this architecture inside `experiments/EXP_XX/models/`?
